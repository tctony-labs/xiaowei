...
impl Service {
    ...
    pub async fn start_services(self: &Arc<Self>, client: Client) -> Result<()> {
        ...
        lifecycle.worker = Some(LifecycleWorker {
            stop,
            thread,
            subscription,
        });
...
mod tests {
    ...
    async fn cleanup_waits_thirty_seconds_repeats_hourly_and_stops_with_lifecycle() {
        ...
        registry
            ...
            .unwrap();
        let client = registry.client(CallContext::trusted("lifecycle"));
        let service =
