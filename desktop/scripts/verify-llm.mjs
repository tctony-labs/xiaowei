// Manual acceptance only; this script does not define or parse its own configuration schema.
// File format and loader: src/main/services/llm/config.ts, shared with the app.
...
import { GenerateRequestSchema, ListModelsRequestSchema, Llm } from "xiaowei-contracts";
import { bindStreamClient } from "xiaowei-gateway";
import { GatewayHost } from "xiaowei-gateway/host";
import { loadConfig, resolveModels } from "../src/main/services/llm/config.ts";
...
try {
  ...
  const { document } = await loadConfig(env);
  const models = resolveModels(document, env);
  if (values.list) {
    console.table(
      document.providers.flatMap((provider) =>
        provider.models.map((model) => ({
          ...
          api: provider.api,
          available: models.some((entry) => entry.id === model.id),
        })),
      ),
    );
  } else {
    const model = models.find((entry) => entry.id === values.model);
    if (!model) throw new Error("model not found or API Key unavailable");
    ...
    if (values.mode === "catalog") {
      const catalog = bindStreamClient(Llm, endpoint);
      const stream = await catalog.modelCatalog(create(ListModelsRequestSchema, { modelRef: model.id }));
      let count = 0;
      try {
