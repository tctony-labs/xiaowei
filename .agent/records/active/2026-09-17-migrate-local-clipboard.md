...
| 数据库访问及结构 | SeaORM 管理业务数据，搜索使用 SQLx；使用旧版实体与迁移结构 | 改用了 rusqlite 和独立设计的表结构；并非复用旧存储层。[gateway 通信](2026-09-18-implement-gateway.md) 已进入计划阶段，统一 [xiaowei-storage](../proposed/2026-09-18-introduce-storage.md)（DB／KV／Config）仍待后续实施，不能视为该偏差已解决 |
