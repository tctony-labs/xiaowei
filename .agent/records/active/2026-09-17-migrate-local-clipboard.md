...
main 就绪后打开数据库并启动监听，退出时停止线程；初次轮询会读取当前剪贴板。通过 changeCount 避免重复读取，读取前后版本不同则下次重试。连续相同内容忽略，内容再次出现时复用已有记录并更新使用时间和次数；复制历史记录时抑制自身回写造成的重复计数。文件 URL 优先于文本读取，每个文件独立写入一个 NSPasteboardItem。 轮询分为读取系统剪贴板和保存历史两个阶段；读取失败记 warning，附件保存或数据库 insert/update 失败记 error，并使用各自的错误文案。同一失败阶段连续发生时只记录一次，阶段变化时重新记录；保存成功后才更新已处理版本与内容哈希，保存失败可在后续轮询重试。
...
- 删除先显示原版确认弹窗，Esc 只关闭弹窗；右键菜单的 Esc 只关闭菜单。列表按旧版最多显示 200 条，不添加“加载更多”按钮；内部通过每页 50 条读取。变更通知先读取不带筛选的全局最新一条记录，通过最近使用时间、排序 ID 和使用次数识别新内容或再次复制；检测到后切回全部（导航显示为「剪贴板」）、清空搜索并选中该条记录。首次读取只建立基线，收藏、备注等元数据刷新保留当前 tab 和仍存在的选中 ID。复制已有条目后置顶并继续选中，列表顺序变化时同步滚动到选中行。
...
文件／图片的右键菜单支持打开、在文件夹中查看和复制路径；多文件记录保留全部文件复制及逐个文件卡片操作，同目录时提供复制文件夹路径。ClipboardBiz 资源操作只接受记录 ID 和可选文件序号，由 Rust 解析真实路径／内容，再通过保留调用权限的 System service 执行宿主动作；页面不提交本地路径。打开多个文件须选择单个卡片；定位多文件记录逐个交给系统文件管理器。Markdown 链接通过 main 校验 HTTP／HTTPS 后调用系统浏览器；渲染模式允许远程 HTTP／HTTPS 图片，图片请求不携带 Referer，CSP 仅放开图片来源。

图片操作不再经过临时导出；复制路径跨应用重启有效，直到对应记录／文件被删除。修复前复制的临时路径需重新复制。长文本全文持久保存于 `clipboard/large_text/<hash>`，数据库只保留前 500 字符摘要，外部查看使用持久文件；普通文本由 Rust 按需原子导出到本 owner 的私有临时目录，保持内容哈希命名和完整 UTF-8。导出在编辑／删除记录后仍保留至 owner 关闭，关闭后统一清理；停止监听不清理外部程序仍可能使用的文件。采集、读取、编辑、长短转换、重复项合并及删除同步维护文件，旧附件在数据库提交成功且无引用后清理。长文本搜索匹配摘要和备注，不扫描文件全文。
...
2026-09-24 补齐捕获后跨 tab 自动选中：通过无筛选的最新记录快照识别最近使用变化，切回全部、清空搜索并选中最新记录；元数据刷新保留当前 tab 与选择。新增 6 项页面回归测试覆盖收藏／图片／文件／自定义分类、搜索清空、重复内容复用和元数据刷新，全部通过；`just check` 通过。Storybook 场景同步更新，本轮未执行其浏览器 play。已确认 hermes 开发监听及 Electron 的路径和 cwd，本次为 renderer 修改，由现有 HMR 加载，真实系统复制后的桌面效果待复验。
...
- 新内容自动选中修复：真实 `ClipboardPage` 的 Storybook `NewItemSelection` 场景覆盖新内容到来、连续新增、第三条回车复制后置顶并保持选中、收藏刷新保留选择，以及搜索范围外新增清空搜索、收藏 tab 捕获后切回全部。预览固定为 800 × 580，检查预览内容和选中行滚动。桌面类型检查、相关文件 Biome 检查与桌面构建通过；当前工作区无运行实例，系统剪贴板采集后的桌面验收待用户启动。
...
### 读取与保存失败区分（2026-09-24）

系统剪贴板读取与历史保存已拆分，分别记录 warning 和 error；保存失败不推进去重状态。同阶段连续错误保留去重，阶段变化会重新记录。17 项剪贴板 Rust 测试通过，包含保存失败后重试同一内容；通过当前 oracle 实例的 just rs 完成原生包重建及重启，实际 tokenizer 缺失故障已验证输出 error 级别的 `Clipboard history save failed`。
