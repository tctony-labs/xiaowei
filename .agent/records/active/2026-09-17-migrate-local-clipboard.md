...
- 删除先显示原版确认弹窗，Esc 只关闭弹窗；右键菜单的 Esc 只关闭菜单。列表按旧版最多显示 200 条，不添加“加载更多”按钮；内部通过每页 50 条读取。变更通知重新读取显示范围；在相同查询和分类下，首条为新增记录或其最近使用时间／次数增加时自动选中首条，其余刷新保留仍存在的选中 ID。复制已有条目后置顶并继续选中，列表顺序变化时同步滚动到选中行；不匹配当前筛选的新内容不改变选择。
