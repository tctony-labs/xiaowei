...
Electron 集成验收目录迁至 gateway/tests/electron，依赖由 gateway/tests/package.json 私有 workspace 包声明；测试资产构建通过，实际 Electron 验收仍显式在已有实例执行。桌面 source 条件解析测试改名并保留于 scripts/tests/desktop-source-resolution.test.mjs，迁移后通过。
