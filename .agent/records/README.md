...
不创建事项。任务在实施中显著扩大到上述范围时，再提出补建事项。

新建或补建任何状态的 record 前，必须先向用户说明拟记录的事项和创建理由，获得确认后再创建。用户已明确要求创建该 record 时无需重复确认；仅同意开发或创建 Plan 不等于确认创建 record。已有 record 的维护、状态迁移和文档同步仍按下述规则执行。
...
- 已有 record 时，实施 Plan 目录必须使用主文档的完整文件名词干。
...
已有 record 的事项，只有正在实施且处于 `active` 时才可以在 `.agent/plans/<initiative-stem>/` 下创建 Plan；没有 record 的任务按 [`Plan README`](../plans/README.md) 创建，不为此强制补建 record。每份 Plan 只覆盖一个可独立验证的实施切片，记录具体文件、执行顺序和验证方法；
