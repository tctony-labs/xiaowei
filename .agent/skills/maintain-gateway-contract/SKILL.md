...
读取 [业务契约原则](../../../contracts/proto/xiaowei/README.md)，核对相关 proto、调用方和实际 owner。用完整的 `package.Service.Method` 查找注册与消费位置；不要把生成配置中的 service 依赖当成运行时注册。桌面装配入口见 [接入说明](../../../docs/gateway-integration.md)。
...
修改生成器或工具链时查阅 [契约生成机制](../../../docs/contracts-generation.md)，普通业务修改无需调整生成器。
...
- 事件须由 owner 显式导出并由消费者订阅，service 绑定不会自动发布事件。涉及事件或流时，按 [运行机制](../../../docs/gateway-runtime.md) 检查 ready、取消、关闭及 owner 替换行为。
