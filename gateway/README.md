...
Rust 业务绑定按 `rust/business_binding_config.rs` 中的模块与 service 列表生成，每个模块只有 `src/gateway_binding.rs`。更新契约后运行 `just gen`；`pnpm gateway:check` 与 Rust 测试检查绑定是否漂移。
...
业务 service 按能力而非部署模块划分：Search 为通用查询与使用反馈，Launcher 管理查询批次／执行和窗口布局，App 提供图标读取，System 提供主题／网页打开，ClipboardBiz 包含页面记录与资源操作，ClipboardDao 提供 Storage 内部记录操作。当前原生搜索 endpoint 承载 Search／App／System.ToggleTheme；main 承载 System.OpenUrl 及 ClipboardBiz 的资源方法，route 无重复注册。具体原则见 [业务契约维护](../contracts/proto/xiaowei/README.md)。
