...
源码 worker 回归、plain Node 构建验证和 Rust napi 通信联调统一运行 `pnpm gateway:test`。LLM service 的产品 worker 入口与 Electron 构建已接线，业务范围和验收见 [LLM provider record](../../.agent/records/active/2026-09-24-llm-provider.md)。
...
每个 renderer client 使用独立的随机 UUID 前缀和递增序号生成订阅／流句柄 ID；热更新重建 client 而 preload 会话仍存活时，新旧句柄不会复用，旧句柄关闭或新请求失败清理也不会取消其他 client 的资源。
